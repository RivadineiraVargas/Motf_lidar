# trajectory_attn_nogate.py  —  Opción C: test limpio de "¿la escena aporta?"
# Escena SIEMPRE activa (use_gate=False): la rama cross-attention objeto→escena
# recibe gradiente completo y PUEDE aprender, rompiendo el candado del gate=0.
# Mismo split + augmentación que trajectory_attn_augmented (que ya ayudó).
# Si esto supera al baseline en val -> la escena aporta. Si no -> es el horizonte.
...
model = dict(
    ...
    scene_dim=64,
    freeze_encoder=True,
    use_gate=False,   # ← escena siempre activa (Opción C)
)
...
work_dir = './work_dirs/trajectory_attn_nogate'
