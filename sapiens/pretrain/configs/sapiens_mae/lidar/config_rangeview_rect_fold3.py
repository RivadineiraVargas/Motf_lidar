# config_rangeview_rect_fold3.py — Re-pre-entrenamiento del encoder MAE SOLO
# en las 20 escenas de TRAIN del fold 3 (sin fuga de las 5 retenidas).
...
train_dataloader = dict(
    ...
    dataset=dict(
        type='CustomDataset',
        data_root='/home/lcad/lidar_sweep_viewer/waymo_clean/range_png_rect/fold3_train',
        with_label=False,
...
# generalización pica ~ép1000.
# Checkpoints c/50 (antes 250): el 08/08 la máquina se cortó en la ép 403 y el
# último checkpoint era el 250 — se perdieron 153 épocas (~2.4h). Con 50 el
# peor caso son ~40 min. max_keep_ckpts=6 acota el disco a ~1 GB.
train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=1000)
default_hooks = dict(checkpoint=dict(interval=50, max_keep_ckpts=6),
                     logger=dict(interval=25))
randomness = dict(seed=0)
work_dir = './work_dirs/rv_rect_fold3'
