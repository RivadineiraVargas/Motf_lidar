...
CKPT_BASELINE = 'work_dirs/clean25_baseline/epoch_100.pth'
CKPT_GATED    = 'work_dirs/clean25_gated/epoch_100.pth'
CKPT_NOGATE   = 'work_dirs/clean25_nogate/epoch_100.pth'
...
def load_attn(ckpt=CKPT_GATED, use_gate=True):
    encoder_cfg = dict(
        type='MAEViT4D',
...
def main():
    ...
    print('\nCargando dataset...')
    # Solo las 25 escenas con LiDAR (train + val); las demás no tienen bin_files
    lidar_scenes = list(TRAIN_SCENES | VAL_SCENES)
    ...
    print('Cargando modelo MOTF (gated attention)...')
    attn_g = load_attn(CKPT_GATED, use_gate=True)
    gate_val = float(torch.tanh(attn_g.scene_gate).item())
    print(f'  Gate actual: tanh(scene_gate) = {gate_val:.4f}')

    print('Cargando modelo MOTF (SIN gate, escena siempre activa)...')
    ...
    res_g    = evaluate(attn_g,   dataset, is_attn=True,  label='MOTF — Gated')
    res_ng   = evaluate(attn_ng,  dataset, is_attn=True,  label='MOTF — SIN gate (escena siempre activa)')

    # Tabla comparativa final
    print(f'\n{"═"*72}')
    print(f'  COMPARATIVA FINAL  —  horizonte 3s  (ADE / FDE en metros, solo XY)')
    print(f'{"═"*72}')
    print(f'  {"":15} {"Baseline":>12}  {"Gated":>12}  {"SinGate":>12}')
    print(f'  {"─"*15}  {"─"*12}  {"─"*12}  {"─"*12}')
    ...
    print(f'{"═"*72}')
    print(f'  Gate (gated model): {gate_val:+.4f}  |  SinGate: escena forzada activa')
    print(f'  * = mejor de los tres en esa métrica')
    print(f'  → Con horizonte 3s y datos limpios: ¿supera la escena al baseline en VAL?')
    print(f'{"═"*72}\n')
